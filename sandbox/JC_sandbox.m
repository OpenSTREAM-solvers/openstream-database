...
% for run = 1%1:23%:20%length(I_test)
parfor run = 1:23
warning('off','all');
% Start without any entryID
...
if P_iteration
    ...
    while (itr<maxitr)
        ...
        if itr ==1 || sign(Power_vec(end)-Power_vec(end-1)) == sign(WLout_vec(end)-WLout_vec(end-1))
            Powerchange(itr) = WLout_vec(itr);
            caseCode(itr) = 1;
        ...
        % if on the same sidecalculate rate of change 
            ...
            newPower = spline(WLout_vec, Power_vec, sign(WLout_vec(itr)).*0.001./2);
            Powerchange(itr) = newPower ./ Power_vec(itr) -1;
            caseCode(itr) = 4;
            % if abs(Powerchange(itr)) > 0.5
            %     Powerchange(itr) = sign(WLout_vec(itr))*max(0.1,abs(WLout_vec(itr))/10);
            %     caseCode(itr) = 6;
            % end
        end

        if abs(Powerchange(itr)) == Inf || isnan(Powerchange(itr)) 
            caseCode(itr) = 5;
            break;
        end   
...
fprintf('Run %2u, entryID %5u, %2u iters, runCodes: ',run, I_test(run), itr-1);
fprintf('%u, ', caseCode(1:end-1));
fprintf('\n');
end
